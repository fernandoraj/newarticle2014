...
about multiple types of data simultaneously sensed in a WSN.
...
selected kinds of sensed data were ``temperature'' and ``humidity'', according
to the type of test. M-magnitude parameter has been configured with the value of
$1.5$ and T-trend was $5\%$, the same as the error threshold. The amount of
initial data sensing (initial slot time) was 70 readings, used for the learning
stage.
...
injected into the network;  {\it
  (iii)} BCWSN-SM-RN, where, after the formation of the clusters based on the
...
strategy, and;  {\it 
  (iv)} BCWSN-FC-CH, which implements the BCWSN, with the scheduling
