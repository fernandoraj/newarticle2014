...
whenever the sink receives "novelties", i.e. new data ($N$), from a RN or CH on a
...
Dimension value (without the new data received / before the new data)
($F_{d}(C_i)$) and the new one ($F_{d}(C'_i$)), where: $C'_i = C_i \cup N$.
If this minimal difference ($|F_d(C'_{\hat{i}}) - F_d(C_{\hat{i}})|$) is greater
than a certain threshold ($\tau$) - defined by the application expert (e.g.
$\tau = 0.03$), then this new data ($N$) is simply rejected as noise.
