...
Let $R_{new}$ be the new reading data to be treated by the sink; let $S_{N}$ be
the sensor which report that novelties (i.e. new reading - $R_{new}$) to the
sink node; let $C_i$ be the sensor cluster on $i-th$ position, formed by a RN or
CH and other sensor members (0 or more); let $SD(C_i)$ be the sensor data of
cluster $C_i$; and let $F_{d}(SD(C_i))$ be the value of Fractal Dimension of the
sensor data of cluster $C_i$.
...
each initial cluster. The idea is to find out which cluster $C_i$ the inclusion
of new sensor data $S_{ND}$ will take the slightest change in its Fractal
Dimension. 

The Minimum Difference Fractal Dimension (MDFD) strategy is as follows:
whenever the sink receives "novelties", i.e. new data ($N$), from a RN or CH on
a given cluster $C_j$, it calculates which cluster $C'_i$ has the minimal
...
($F_{d}(C_i)$) and the new one ($F_{d}(C'_i$)), where: $C'_i = C_i \cup N$.
If this minimal difference, min($|F_d(C'_{\hat{i}}) - F_d(C_{\hat{i}})|$), is
greater than a certain threshold ($\tau$) - defined by the application expert
(e.g. $\tau = 0.03$), then this new data $N$ is simply rejected as noise.
On the other hand, if the cluster $C'_i$ is the same $C_j$ (i.e. $i=j$), the
sink just add the new data $N$ to the corresponding sensor in this cluster.
Otherwise, the sink must remove this sensor node (with new data $N$) from its
old cluster $C_j$ and add it to the new cluster $(C_i)$.
...
\begin{algorithm}
 \SetAlgoLined
 \LinesNumbered
 \KwData{input parameters: set of all K current sensor clusters (\{$C_i \vert 1 \leq i \leq K$\}), values of Fractal Dimension of all current sensor clusters ($F_d(C_i)$), sensor node($S_N$) with new data ($N$)}
 \KwResult{processes Fractal Clustering}
 Sink Node receives new data ($N$) from sensor ($S_N$) of cluster $C_j$\;
 \For{i = 1,\ldots,K}{
	  Let $C'_i = C_i \cup N$\;
	  Compute $F_d(C'_i)$\;
	  Find $\hat{i} = min(|F_d(C'_i) - F_d(C_i)|)$\;
	  \eIf{$|F_d(C'_{\hat{i}}) - F_d(C_{\hat{i}})| > \tau$}{
	  	Discard $N$ as noise\;
	  }{
	  	\eIf{$j \neq \hat{i}$}{
	  		Remove $S_N$ from cluster $C_j$\;
	  		Place $S_N$ in cluster $C_\hat{i}$\;
	  	}{
	    	Replace $S_N$ in cluster $C_\hat{i}$\;
	    }
	  }
  }
