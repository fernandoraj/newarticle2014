...
trend of sensed data. For clusters maintenance, we propose an approach, which
employees the notion of Fractal Dimension to find the best configuration for
...
given node have very nearly values (the principle of temporal locality). This 
peculiarity makes possible to predict (with a given error threshold) future sensed 
values based on data collected in the past. On the other hand, traditional
...
also known as "the principle of spatial locality". Thus, it can be inferred that 
from the readings of a sensor located in a given region, it is possible to estimate 
the another sensors reading values nearby.
\vspace*{-.3cm}
...
probably that the temperature measurements of sensors in those spatially separated
...
applicable. For that reason, we assert that using sensor clustering strategy
based on \textit{Behavioral Correlation} is the best method for accuracy results, 
since it provides better solution for both the traditional case and the case 
illustrated in Figure \ref{fig:contour_lines}.
\vspace*{-.3cm}
...
single cluster, towards existing spatial correlation techniques.
\vspace*{-.3cm}
...
Correlation} (TC) (a.k.a. {\it Behavio-Temporal Correlation} - BTC).
\vspace*{-.3cm}
%[\To Review]
