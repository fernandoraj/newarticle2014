...
For clusters maintenance, BCWSN implements the notion of Fractal Dimension to dynamically find the best configuration for
...
WSN based on the notion of {\it Behavioral Correlation}. BCWSN
is based on clustering sensors initially by means of behavioral correlation
(described in Step 2) among several physical phenomena (e.g., temperature, humidity, light), which in turn is computed from the time series
...
\begin{center}
	\includegraphics[scale=0.33]{BCWSN-TotNumMsgxRound-PB-2Appr-Hum.png}
	 \vspace*{-.6cm}
    \caption{Number of Messages per Round (Humidity)}
