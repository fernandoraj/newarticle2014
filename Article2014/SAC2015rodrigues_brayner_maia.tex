...
The results show that BCWSN can reduce the number of messages injected into the
network up to 94.80\% over a naive approach (no clustering, no prediction and no
suppression) and up to 57.24\% when compared to approaches implementing temporal
...
Adaga-P* \cite{MaiaSAC2013, MaiaACR2013} constructs sensor-field models which
...
Nonetheless, BCWSN approach is able to dynamically adjust the accuracy by
reducing the RMSE through the use of Fractal Clustering (FC).
\vspace*{-.3cm}
...
higher than the energy consumed to execute BCWSN for gathering ``temperature''
