...
% \title{Using Fractal Clustering to explore Behavioral Correlation: A new
% approach to reduce energy consumption in Wireless Sensor Networks}
...
% Sensors that send new
% data (novelties) are allocated in clusters therein by the sink node, which the
% smallest change in value of Fractal Dimension occurs, making the choice of the
% new most appropriate cluster nodes (with new data readings received by sink
% node) the most accurate as possible (known as Fractal Clustering - FC). 
